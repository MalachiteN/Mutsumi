 ...
 */
export function formatShellOutput(snap: ShellTaskSnapshot, opts: { showExit: boolean }): string {
    const parts: string[] = [];
...
export class ShellTask {
    ...
    constructor(opts: ShellTaskOptions) {
        ...
        this.child.stdout?.on('data', (d: Buffer) => { this.stdoutBuf += d.toString(); });
        this.child.stderr?.on('data', (d: Buffer) => { this.stderrBuf += d.toString(); });
    ...
    get isRunning(): boolean { return this.exitCode === null && !this.aborted; }
    get isExited(): boolean { return this.exitCode !== null; }
    ...
    private fireExit(): void {
        ...
        this.onExitCbs = [];
        cbs.forEach(fn => { try { fn(); } catch { /* ignore */ } });
    }
}
