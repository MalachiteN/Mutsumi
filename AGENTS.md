...
- 边栏 Context 树的文件操作会回溯改写 Ghost Block：**Remove File** 从所有 Cell 的 ghost block 中剥离该 key 的全部条目并删除版本跟踪项；**Prune Old Versions** 保留版本跟踪项，仅剥离非最新版本的条目。Notebook 工具栏另有一键 **Prune Old Ghost Versions** 命令，对会话内所有文件引用批量执行版本裁剪。这些操作都实际缩短上下文，并使最早被修改 Cell 之后的前缀缓存失效；不操作则保持缓存命中。
