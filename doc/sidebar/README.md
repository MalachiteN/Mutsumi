...
Sidebar模块负责Mutsumi插件的侧边栏视图功能，包含两个主要树视图：
...
1. **AgentSidebarProvider** 作为主控制器，创建并注册两个树视图
2. **AgentTreeDataProvider** 从 `AgentOrchestrator` 获取Agent数据，构建层级结构
3. **ApprovalTreeDataProvider** 从 `approvalManager` 获取审批请求数据
4. 两个Provider分别使用各自的TreeItem类渲染节点
...
| approvalTreeProvider.ts | ApprovalTreeDataProvider | 类 | 审批请求树数据提供者 |
...
| [agentTreeProvider.ts](./agentTreeProvider.md) | Agent树数据管理，实现层级结构展示 |
| [approvalTreeItem.ts](./approvalTreeItem.md) | 审批请求节点定义，包含状态图标和提示 |
| [approvalTreeProvider.ts](./approvalTreeProvider.md) | 审批请求数据管理，自动同步变更 |
